...
function downvoteComment( url, request ) {
  return upvoteOrDownvoteCommentOrArticle( url, request, {type: "comments", action: "downvote"} );
}

function upvoteComment( url, request ) {
  return upvoteOrDownvoteCommentOrArticle( url, request, {type: "comments", action: "upvote"} );
}

function upvoteOrDownvoteCommentOrArticle ( url, request, selector ) {
  const id = getIdFromUrl ( url );
  const username = request.body && request.body.username;
  let savedItem = database[selector.type][id];
  const response = {};

  if (savedItem && database.users[username]) {
    if ( "upvote" === selector.action ) {
      savedItem = upvote(savedItem, username);
    } else if ( "downvote" ){
      savedItem = downvote(savedItem, username);
    }

    if ( "comments" === selector.type ) {
      response.body = {comment: savedItem};
    } else if ( "articles" === selector.type ){
      response.body = {article: savedItem};
    }
    response.status = 200;
  } else {
    response.status = 400;
...
function deleteComment( url, request ){
  ...
  if (savedComment) {
    ...
    //remove the commentId from the user's list of comments$
    const userCommentIds = database.users[savedComment.username].commentIds;
    userCommentIds.splice(userCommentIds.indexOf(id), 1);
    //remove the commentID from article's list of comments
...
//haven't refactored upvoteArticle to use
//return upvoteOrDownvoteCommentOrArticle( url, request, {type: "articles", action: "upvote"}
//because I cannot get index.html to work locally
//so I cannot check if it works
...
//haven't refactored downvoteArticle to use
//return upvoteOrDownvoteCommentOrArticle( url, request, {type: "articles", action: "downvote"}
//because I cannot get index.html to work locally
//so I cannot check if it works
